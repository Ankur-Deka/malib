...
# malib/multiagent/environment.py and malib/multiagent/core.py are also env files
class Scenario(BaseScenario):   ## pretty much useless to import basescenario
    def make_world(self):
        world = World()
        ...
        self.collision_rew = True # Do you wanna use collision avoidance reward?
        return world
    ...
    def agent_reward(self, agent, world):
        ...
        # let's give negative of convex hull reward
        # rew = -self.convex_hull_reward(agent, adversaries, th = 1)
        
        # maximize distance from nearest adversary
        rew += 0.1 * min([np.sqrt(np.sum(np.square(agent.state.p_pos - adv.state.p_pos))) for adv in adversaries])
        # print(rew)
        # if agent.collide:
        #     # negative reward for collision with adverary
        #     for a in adversaries:
        #         if self.is_collision(a, agent):
        #             rew -= 10
            
            # ## negatively rewarded for colliding with obstacles
    ...
    def convex_hull_reward(self, agent, adversaries, th):
    ...
    def adversary_reward(self, agent, world):
        ...
        # Let's give the convex hull reward
        rew += 10*self.convex_hull_reward(agents[0], adversaries, th = 1)
        ...
        # give individual reward to each adversary
        rew -= 0.1 * min([np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos))) for a in agents])
        
        # print('adversary reward')
        # print('convex hull {}, dist {}'.format(10*self.convex_hull_reward(agents[0], adversaries, th = 1), 0.1 * min([np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos))) for a in agents])))
        ...
        if self.collision_rew:
            ## negatively rewarded for colliding with other adversaries
            for adv in adversaries:
                if agent.name != adv.name:
